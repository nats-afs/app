...
const actions = {
  ...
  loginWithTwitter: (context, payload) => {
    firebaseAuth().signInWithPopup(new firebaseAuth.TwitterAuthProvider()).then(() => {
      context.commit(AUTH_STATUS_CHANGE)
      router.push('/')
    }).catch(error => {
      console.log(error)
  ...
  // loginWithPhone: (context, payload) => {
  //   firebaseAuth().signInWithPhoneNumber(payload.phoneNumber, payload.applicationVerifier).then(() => {
  //     context.commit(AUTH_STATUS_CHANGE)
  //     router.push('/')
  //   }).catch(error => {
  //     console.log(error)
